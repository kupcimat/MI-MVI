...
to setup
  ...
  if grass? [
    ask patches [
      ...
      set pcolor one-of [green brown gray]
    ]
  ]
...
to use-sample-map
  ...
  ask patches [ if random 10 < 3 [set pcolor brown] ]
  ask patches [
    let x pxcor
    let y pycor
    if (x * x / 2 + y * y / 5 < 20 and x * x + y * y / 3 > 5 and x > 0) or
       (x * x + (y - 6) * (y - 6) < 8) or (x * x + (y + 6) * (y + 6) < 8) or
       ((x + 16)*(x + 16) + (y - 16)*(y - 16) < 20) or
       ((x - 16)*(x - 16) + (y - 16)*(y - 16) < 20) or
       ((x + 16)*(x + 16) + (y + 16)*(y + 16) < 20) or
       ((x - 16)*(x - 16) + (y + 16)*(y + 16) < 20)
    [ set pcolor gray ]
  ]
end

to go
  ;; if not any? sheep or not any? wolves
  ;; [ print-sheep-fitness
  ;;   stop ]
  ...
  ask sheep [
    ...
    move-turtle "move-sheep"
   ]

  ask wolves [
    set energy energy - 1  ;; wolves lose energy as they move
    catch-sheep
    move-turtle "move-wolf"
   ]
  ...
  ask sheep [ reproduce-sheep ]
  ask wolves [ reproduce-wolves ]
...
to get-slaughtered ;; turtle procedure
  let animal turtles-on patches with [ pcolor = gray ]   
  ask animal [ die ]   
end
...
to move-turtle [ cmd ]
  reset-timer
  let command ( runresult cmd )
  ifelse timer <= 0.1
    [
      ifelse item 1 command < 1.0001
        [
          rt item 0 command
          fd item 1 command 
        ]
...
;;to print-sheep-fitness  ;; final result
;;  output-show count sheep - count wolves
;;end
...
to display-labels  ;; turtle procedure
